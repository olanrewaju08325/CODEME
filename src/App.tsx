...
import { Award, Loader2, Bell } from 'lucide-react'
...
function App() {
  ...
  const checkCertificateStatus = async (userId: string) => {
    try {
      ...
      const { data: quizzes } = await supabase
        ...
        .eq('modules.course_id', 'wd101')

      if (quizzes && attempts) {
        const passedQuizzes = quizzes.filter(q =>
          attempts.some(a => a.quiz_id === q.id && a.passed)
        ).length
        setAllPassed(passedQuizzes === 6)
      }
    } catch (e) {
      console.error(e)
    }
    ...
    try {
      ...
      if (!data.last_active_date) {
        const { data: upData } = await supabase
          ...
          .eq('id', activeSession.user.id)
          .select().single()
        if (upData) updatedProfile = upData
      } else {
        const diffDays = Math.floor((new Date(todayStr).getTime() - new Date(data.last_active_date).getTime()) / 86400000)
        if (diffDays === 1) {
          const { data: upData } = await supabase
            .from('profiles')
            .update({ streak_count: data.streak_count + 1, last_active_date: todayStr })
            .eq('id', activeSession.user.id)
            .select().single()
          if (upData) updatedProfile = upData
        } else if (diffDays > 1) {
      ...
      checkCertificateStatus(activeSession.user.id)
      fetchUnreadCount(activeSession.user.id)
  ...
  const navigate = (view: string) => {
    setCurrentView(view)
    if (session) {
      checkCertificateStatus(session.user.id)
    ...
    <>
      <Suspense fallback={
        ...
        {currentView === 'unverified' && (
          <div className="full-screen-view theme-dark" style={{ background: 'radial-gradient(circle at center, #1b1030 0%, #07060d 100%)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
...
// ─────────────────────────────────────────────
function NotificationsView({ session, onNavigate, onRead }: { session: any; onNavigate: (v: string) => void; onRead: () => void }) {
...
function ProfileView({ session, profile, onNavigate, onSignOut, isDarkMode, onToggleDarkMode, allPassed }: any) {
  ...
  return (
    <div className="full-screen-view" style={{ paddingBottom: '80px' }}>
      ...
      {/* Profile Card */}
      <div className="gradient-card" style={{ padding: '24px', marginBottom: '16px', textAlign: 'center', borderRadius: '16px', background: 'linear-gradient(135deg, #1b1030, #0c0a1e)' }}>
