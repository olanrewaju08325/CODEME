...
export const AdminPortal: React.FC<AdminPortalProps> = ({ session, onSignOut }) => {
  ...
  const [maxBatchSizeInput, setMaxBatchSizeInput] = useState('25')
  ...
  const handleApproveApplication = async (app: any) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('create_student_account', {
        ...
        p_course_id: app.course_id || 'wd101'
      })
      if (error) throw error
      // Mark application as approved
      await supabase.from('enrollment_applications').update({ status: 'approved' }).eq('id', app.id)
      setMessage({ type: 'success', text: `✅ Account created! Student ID: ${data}. Temp password: ${tempPassword} — Send this to ${app.email} / ${app.phone}` })
  ...
  const handleRejectApplication = async (id: string) => {
    setActionLoading(true)
    try {
      await supabase.from('enrollment_applications').update({ status: 'rejected' }).eq('id', id)
  ...
  const handleCreateStudentAccount = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc('create_student_account', {
        ...
        p_course_id: newStudentCourse
      })
      if (error) throw error
      setCreatedStudentId(data as string)
      setMessage({ type: 'success', text: `Account created! Student ID: ${data}` })
      setNewStudentEmail('')
      setNewStudentName('')
  ...
  const fetchWaitlistData = async () => {
    try {
      const { data: waitlistData } = await supabase
        ...
        .select('*, profiles:profiles!student_enrollments_student_id_fkey(full_name, student_id, email)')
      ...
      const { data: settingData } = await supabase
        .from('app_settings')
        .select('value')
        .eq('key', 'max_batch_size')
        .single()
      if (settingData) {
        setMaxBatchSizeInput(settingData.value)
      }
    } catch (e) {
  ...
  const handleUpdateBatchCapacity = async (e: React.FormEvent) => {
    e.preventDefault()
    setActionLoading(true)
    setMessage(null)
    try {
      const sizeVal = parseInt(maxBatchSizeInput)
      if (isNaN(sizeVal) || sizeVal <= 0) {
        throw new Error('Please enter a valid positive number for capacity limit.')
      }
      
      const { error } = await supabase
        .from('app_settings')
        .upsert({ key: 'max_batch_size', value: sizeVal.toString() })

      if (error) throw error
      setMessage({ type: 'success', text: `Max Batch Capacity successfully updated to ${sizeVal}!` })
      fetchWaitlistData()
    } catch (e: any) {
      setMessage({ type: 'error', text: e.message || 'Error updating settings' })
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleFlagAI = async (id: string, isFlagged: boolean) => {
    try {
      const { error } = await supabase
        ...
        .update({ is_ai_flagged: isFlagged })
        .eq('id', id)
      if (error) throw error
      fetchQueues()
  ...
  const [certForm, setCertForm] = useState({ course_id: '', template_name: '', primary_color: '#0C4A8C', signatory_name: 'Olamide A.O' })
  const [courses, setCourses] = useState<any[]>([])
  const [showCertTemplateModal, setShowCertTemplateModal] = useState(false)
  const [certTemplateForm, setCertTemplateForm] = useState<any>(null)
  ...
  const handleSaveCertTemplate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!certForm.course_id || !certForm.template_name) return
    setActionLoading(true)
    setMessage(null)
    try {
      const { error } = await supabase.from('certificate_templates').upsert({
        course_id: certForm.course_id,
        template_name: certForm.template_name,
        primary_color: certForm.primary_color,
        signatory_name: certForm.signatory_name
      }, { onConflict: 'course_id' })
      if (error) throw error
      setMessage({ type: 'success', text: 'Certificate template saved!' })
      fetchCertTemplates()
    } catch(e: any) {
      setMessage({ type: 'error', text: e.message })
    } finally {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const { count: wd101Count } = await supabase
        .from('student_enrollments')
        .select('*', { count: 'exact', head: true })
        .eq('course_id', 'wd101')
      ...
      setMetrics({
        ...
        activeWD101: wd101Count || 0,
        pendingPayments: paymentsCount || 0,
        pendingGrading: gradingCount || 0
      })
    } catch (e) {
      console.error(e)
    }
  ...
  const fetchQueues = async () => {
    try {
      ...
      const { data: gradData } = await supabase
        .from('assignment_submissions')
        .select('*, profiles:profiles!assignment_submissions_student_id_fkey(full_name, student_id), assignments(title, modules(order_index))')
        .eq('status', 'pending')
      ...
      const { data: payData } = await supabase
        .from('exam_payment_verifications')
        .select('*, profiles(full_name, student_id), quizzes(title)')
        .eq('status', 'pending')
        .order('created_at', { ascending: true })
      setPaymentsQueue(payData || [])
      ...
      // Fetch modules for syllabus assignment
      ...
      if (modData && modData.length > 0) {
        setSelectedModuleId(modData[0].id)
      }
    } catch (e) {
      console.error(e)
    }
  ...
  const handleModeratePost = async (id: string, action: 'approve' | 'delete') => {
    ...
    try {
      if (action === 'approve') {
        const { error } = await supabase.from('forum_posts').update({ status: 'approved' }).eq('id', id)
        if (error) throw error
  ...
  const handleCreateLesson = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!lessonTitle.trim() || !lessonContent.trim() || !selectedModuleId) return
    setActionLoading(true)
    setMessage(null)
    try {
      const { error } = await supabase
      ...
      setMessage({ type: 'error', text: e.message || 'Error inserting lesson' })
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const fetchStudentHistory = async (studentId: string) => {
    try {
      const { data: progress } = await supabase
        ...
        .select('*, lessons(title, modules(order_index))')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="app-content" style={{ padding: '16px' }}>
        ...
        {activeTab === 'waitlist' && role === 'admin' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <h3 style={{ fontFamily: 'var(--font-headings)', fontSize: '1.1rem', fontWeight: 800 }}>Intake Batch Size Config</h3>
              <form onSubmit={handleUpdateBatchCapacity} style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                <div className="form-group" style={{ flex: 1, marginBottom: 0 }}>
                  <label style={{ fontSize: '0.7rem' }}>MAX BATCH SIZE LIMIT</label>
                  <input 
                    type="number" 
                    className="input-field" 
                    value={maxBatchSizeInput}
                    onChange={(e) => setMaxBatchSizeInput(e.target.value)}
                    required
                    style={{ height: '38px', minHeight: '38px' }}
                  />
                </div>
                <button type="submit" className="btn btn-primary" style={{ height: '38px', minHeight: '38px', padding: '0 16px', cursor: 'pointer' }} disabled={actionLoading}>
                  Update Capacity
                </button>
              </form>
            </div>
            ...
            {waitlistQueue.length === 0 ? (
              ...
              waitlistQueue.map(item => (
                <div key={item.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h4 style={{ fontSize: '0.9rem', fontWeight: 700 }}>{item.profiles?.full_name || 'Anonymous'}</h4>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>ID: {item.profiles?.student_id || 'N/A'}</p>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Email: {item.profiles?.email || 'N/A'}</p>
                  </div>
                  ...
                  <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <button
                        className="btn btn-secondary"
                        onClick={() => {
                          const existing = certTemplates.find((t: any) => t.course_id === c.id)
                          if (existing) {
                            setCertTemplateForm(existing)
                          } else {
                            setCertTemplateForm({ course_id: c.id, template_name: 'Standard Template', primary_color: '#0d182e', logo_url: '', signatory_name: 'CODEME Director', signatory_title: 'Director of Education' })
                          }
                          setShowCertTemplateModal(true)
                        }}
                        style={{ padding: '6px 14px', fontSize: '0.8rem' }}
                      >
                        <Settings size={14} style={{ marginRight: '5px' }} /> Edit
                      </button>
                    </div>
                  </div>
                ))}
