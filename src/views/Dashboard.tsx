...
import { BookOpen, Bell, Flame, Award, Calendar, Volume2, ChevronRight, LogOut, AlertTriangle, Sun, Moon, Trophy, Star, Zap, Lock, Users, Settings, Fingerprint, CheckCircle } from 'lucide-react'
    ...
    const fetchData = async () => {
      try {
        ...
        const { data: badgeData } = await supabase
          ...
          .select('*')
          .eq('student_id', session.user.id)
        const currentBadges = badgeData || []
        setBadges(currentBadges)

        // 7. Auto check and award achievements — wrap in its own try/catch so
        // connection errors don't block the whole dashboard
        let passedCount = 0
        try {
          const { data: passedQuizzes } = await supabase
            ...
            .eq('student_id', session.user.id)
            .eq('passed', true)
          const uniquePassedIds = Array.from(new Set((passedQuizzes || []).map((q: any) => q.quiz_id)))
          passedCount = uniquePassedIds.length
        } catch (_) { /* ignore connection errors */ }

        const badgesToInsert = []
        if (!currentBadges.some(b => b.badge_name === 'Welcome Scholar')) {
          badgesToInsert.push({ student_id: session.user.id, badge_name: 'Welcome Scholar' })
        }
        if (passedCount >= 1 && !currentBadges.some(b => b.badge_name === 'HTML Pioneer')) {
          badgesToInsert.push({ student_id: session.user.id, badge_name: 'HTML Pioneer' })
        }
        if (passedCount >= 3 && !currentBadges.some(b => b.badge_name === 'Form Master')) {
          badgesToInsert.push({ student_id: session.user.id, badge_name: 'Form Master' })
        }
        if (passedCount >= 5 && !currentBadges.some(b => b.badge_name === 'Semantic Knight')) {
          badgesToInsert.push({ student_id: session.user.id, badge_name: 'Semantic Knight' })
        }
        if (passedCount === 6 && !currentBadges.some(b => b.badge_name === 'Web Graduate')) {
          badgesToInsert.push({ student_id: session.user.id, badge_name: 'Web Graduate' })
        }

        if (badgesToInsert.length > 0) {
          await supabase.from('student_badges').insert(badgesToInsert)
        ...
        // 8. Fetch Leaderboard comparison
        const { data: leaderboardData } = await supabase
          .from('profiles')
          .select('id, full_name, student_id, streak_count, avatar_url, quiz_attempts(passed)')
          .eq('role', 'student')
        
        const sorted = (leaderboardData || []).map((st: any) => {
          const passedList = (st.quiz_attempts || []).filter((a: any) => a.passed)
          return {
            fullName: st.full_name,
            studentId: st.student_id,
            streak: st.streak_count || 1,
            avatarUrl: st.avatar_url,
            passedCount: passedList.length
          }
        })
        .sort((a, b) => b.passedCount - a.passedCount || b.streak - a.streak)
        .slice(0, 5)
        // 9. Fetch all courses for roadmap
        ...
        // 10. Build adaptive recommendations based on progress
        if (coursesData && coursesData.length > 0) {
          const recs = []
          if (completedCount >= 4) recs.push(coursesData.find((c: any) => c.id === 'css') || null)
          if (completedCount >= 8) recs.push(coursesData.find((c: any) => c.id === 'js') || null)
          if (passedCount >= 3) recs.push(coursesData.find((c: any) => c.id === 'react') || null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      >
        ...
        <motion.div 
          variants={itemVariants}
          style={{ display: 'flex', gap: '12px', alignItems: 'center', backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', padding: '12px', borderRadius: '12px' }}
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {allCourses.filter(c => c.id !== 'wd101').map(course => (
              <div 
                ...
                className="card"
                style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '14px', borderRadius: '12px', cursor: 'pointer', hover: { transform: 'translateY(-2px)' } }}
              >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <h4 style={{ fontSize: '0.85rem', fontWeight: 600 }}>Meet with Instructor (Webinar)</h4>
            <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Saturday at 4:00 PM (West Africa Time)</p>
          </div>
        ...
        <motion.div variants={itemVariants}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '10px', fontFamily: 'var(--font-headings)' }}>My Achieved Badges</h3>
          <div className="card" style={{ display: 'flex', gap: '16px', overflowX: 'auto', padding: '16px', scrollbarWidth: 'none' }}>
            {badges.length === 0 ? (
              <p style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>No badges earned yet. Complete your quizzes to unlock achievements!</p>
            ) : (
              badges.map((b, idx) => (
                <div key={idx} style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', width: '70px' }}>
                  ...
                  <span style={{ fontSize: '0.6rem', fontWeight: 700, textAlign: 'center', textOverflow: 'ellipsis', overflow: 'hidden', width: '100%' }}>{b.badge_name}</span>
                </div>
              ))
            )}
          </div>
          ...
          <div className="card" style={{ backgroundColor: 'var(--bg-secondary)', width: '100%', maxWidth: '480px', borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderRadius: '20px 20px 0 0', maxHeight: '85vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0' }}>
            ...
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                ...
                {biometricSupported ? (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: biometricEnabled ? 'rgba(16,185,129,0.1)' : 'var(--bg-primary)', borderRadius: '12px', border: `1px solid ${biometricEnabled ? 'rgba(16,185,129,0.4)' : 'var(--border-color)'}` }}>
                    <div>
                      <p style={{ fontWeight: 600, fontSize: '0.85rem' }}>Fingerprint / Face ID</p>
                      <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '2px' }}>Use biometrics to sign in instantly on this device</p>
                    </div>
                            ...
                            try {
                              const credential = await navigator.credentials.create({
                                publicKey: {
                                  ...
                                  rp: { name: 'CodeMe Academy' },
                                  user: { id: new Uint8Array(16), name: session.user.email, displayName: profile?.full_name || 'Student' },
                                  pubKeyCredParams: [{ type: 'public-key', alg: -7 }],
                                  authenticatorSelection: { authenticatorAttachment: 'platform', userVerification: 'required' },
                                  timeout: 60000
                                }
                              })
                              if (credential) setBiometricEnabled(true)
                            } catch (_) { setBiometricEnabled(false) }
                          } else {
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    ...
                    { key: 'push', label: 'Push Notifications', sub: 'Live class reminders and deadlines', icon: '🔔' },
